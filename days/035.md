# Day 035: Hyperparameter Tuning with Optuna
...
4. The end state must include:
   ...
   - The Optuna study is directed so that `best_params` corresponds to the highest-F1 trial in the search space — not the lowest.
...
def objective(trial, X, y):
    ...
    # BUG: no MLflow logging — trial never lands on the tracking server
    return score
...
def main():
    ...
    study = optuna.create_study(
        direction="minimize", study_name=EXPERIMENT_NAME   # BUG: f1 is "higher is better"
    )
...
**1. Direction** — `minimize` makes Optuna chase the *worst* F1; switch to `maximize`:
```python
study = optuna.create_study(
...
**2. Log every trial to MLflow** inside `objective()`, right before returning the score:
```python
def objective(trial, X, y):
    n_estimators = trial.suggest_int("n_estimators", 50, 500)
    max_depth = trial.suggest_int("max_depth", 3, 20)
...
```
[I ...] Trial 2 finished with value: 0.47142857142857136 and parameters: {'n_estimators': 267, 'max_depth': 17}. Best is trial 2 with value: 0.47142857142857136.
...
best params: {'n_estimators': 267, 'max_depth': 17}
best f1: 0.471429
wrote /root/code/fraud-detection/configs/best_params.yaml
```

All 20 trials logged as separate MLflow runs, and `best_params` tracks the highest F1 in the search (trial 2).

---

## Key Takeaways

### Optuna's `direction` has to match what the metric means
Optuna doesn't know whether your objective is "lower is better" (like loss) or "higher is better" (like F1) — that's purely the `direction` argument. Get it backwards and the study still runs cleanly, prints a "best" trial, and writes a YAML file — it's just optimising for the worst-performing configuration in your search space without ever erroring out.

### Logging has to happen inside the function Optuna actually calls
`objective()` runs once per trial — that's the only place that has each trial's specific hyperparameters and score in scope. Logging from anywhere else (like after `study.optimize()` finishes) would only ever see the final state, not the full per-trial history the Compare view needs.
